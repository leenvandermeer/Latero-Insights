...
function generatePipelines(): unknown[] {
  ...
  for (const date of dates) {
    for (const { dataset, source_system } of DATASETS) {
      // ~33% coverage per dataset-day to yield ~120 total records
...
function generateLineage(): unknown[] {
  ...
  for (const { dataset } of DATASETS) {
    ...
    for (const layer of layers) {
      ...
      // Column-level lineage for bronze_to_silver and silver_to_gold
...
function main(): void {
  console.log(`Seeding cache for date range ${FROM} to ${TO}...`);
  ...
  const endpoints: Array<{ name: string; generator: () => unknown[] }> = [
    { name: "pipelines", generator: generatePipelines },
    { name: "quality", generator: generateQuality },
    { name: "lineage", generator: generateLineage },
  ];

  for (const { name, generator } of endpoints) {
    ...
    console.log(`  ✓ ${name}: ${data.length} records → .cache/${key}.json`);
  }

  // Write settings.json with cacheOnly: true — preserve any existing Databricks credentials
  ...
  const settings = {
    databricksHost: existing.databricksHost ?? "",
    databricksToken: existing.databricksToken ?? "",
    databricksWarehouseId: existing.databricksWarehouseId ?? "",
    databricksCatalog: existing.databricksCatalog ?? "workspace",
    databricksSchema: existing.databricksSchema ?? "meta",
    cacheTtlSeconds: existing.cacheTtlSeconds ?? 604800,
    cacheOnly: true,
  };
  writeFileSync(settingsPath, JSON.stringify(settings, null, 2), "utf-8");
  console.log("  ✓ settings.json → cache-only mode enabled (credentials preserved)");

  console.log("Done! Start with: npm run dev");
}
