...
export async function GET(request: NextRequest) {
  ...
  if (isCacheOnly()) {
    const cached = getFromCache<LineageEntity[]>(CACHE_KEY, cacheParams);
    if (cached) {
  ...
  try {
    const entities = await getLineageEntitiesFromSaaS(installationId);
    writeToCache(CACHE_KEY, cacheParams, entities);
