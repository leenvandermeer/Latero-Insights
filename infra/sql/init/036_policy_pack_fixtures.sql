-- LADR-078 / WP-405: Policy Pack Fixtures
-- Standaard policy packs + policies voor ESG/CSRD, BCBS-239 en Data Mesh Governance.
-- Idempotent: ON CONFLICT DO NOTHING op alle records.
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-esg-owner', i.installation_id, 'system-esg-csrd',
  'Data Product Owner Required',
  'Every data product used for ESG reporting must have an owner',
  '{"condition":"owner_missing"}',
  '{"all":true}',
  'warn', true
FROM insights_installations i
ON CONFLICT DO NOTHING;
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-esg-quality', i.installation_id, 'system-esg-csrd',
  'Quality Pass Rate ≥ 95%',
  'ESG data products must maintain a quality pass rate of at least 95%',
  '{"condition":"quality_below_threshold","threshold":0.95}',
  '{"all":true}',
  'warn', true
FROM insights_installations i
ON CONFLICT DO NOTHING;
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-esg-lineage', i.installation_id, 'system-esg-csrd',
  'Full Lineage Required',
  'ESG data products must have documented lineage',
  '{"condition":"no_lineage"}',
  '{"all":true}',
  'warn', true
FROM insights_installations i
ON CONFLICT DO NOTHING;
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-bcbs-owner', i.installation_id, 'system-bcbs239',
  'Data Owner Required',
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-bcbs-incidents', i.installation_id, 'system-bcbs239',
  'No Open Critical Incidents',
  'Risk data products must not have open critical incidents',
  '{"condition":"open_incidents","severity":"critical"}',
  '{"all":true}',
  'block', true
FROM insights_installations i
ON CONFLICT DO NOTHING;
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-mesh-owner', i.installation_id, 'system-data-mesh',
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-mesh-quality', i.installation_id, 'system-data-mesh',
  'Quality Pass Rate ≥ 90%',
  'Data products must meet minimum quality standards',
  '{"condition":"quality_below_threshold","threshold":0.90}',
  '{"all":true}',
  'warn', true
FROM insights_installations i
ON CONFLICT DO NOTHING;
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-mesh-contract', i.installation_id, 'system-data-mesh',
  'Data Contract Required',
...
INSERT INTO meta.policies
  (id, installation_id, pack_id, name, description, rule, scope, action, active)
SELECT
  'system-mesh-lineage', i.installation_id, 'system-data-mesh',
  'Lineage Required',
